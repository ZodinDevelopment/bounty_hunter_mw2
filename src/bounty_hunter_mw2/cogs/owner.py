...
class Owner(commands.Cog, name="owner"):
    ...
    async def sync(self, context: Context, scope: Literal['global', 'guild']) -> None:
        ...
        if scope == "global":
            ...
            embed = discord.Embed(
                description="Slash Commands have been fully synchronized.",
                color=0x9C84EF
            )
            ...
            return
        embed = discord.Embed(
            description="Invalid scope provided! Must be 'global' or 'guild'.",
            color=0xE02B2B
        )
        await context.send(embed=embed)
    ...
    async def load(self, context: Context, cog: str) -> None:
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"Could not load cog {cog}",
                color=0xE02B2B
            )
            await context.send(embed=embed)
            return
    ...
    async def say(self, context: Context, *, message: str) -> None:
        ...
        :param context: The command context
        :param message: The message you want the bot to repeat.
        """
    ...
    @app_commands.describe(message="The message you want to embed.")
    @checks.is_owner()
    async def embed(self, context: Context, *, message: str) -> None:
        """
        The bot says something but within embeds.

        :param context: The command context.
        :param message: The message you want embedded.
        """
        embed = discord.Embed(
            description=message,
            color=0x9C84EF
    ...
    @blacklist.command(
        base="blacklist",
        name="add",
        description="Lets you add a user to the bot's blacklist."
    )
    @app_commands.describe(user="The user that should be added to the blacklist.")
    @checks.is_owner()
    async def blacklist_add(self, context: Context, user: discord.User) -> None:
        """
        Lets you add a user tothe bot's blacklist.
        ...
        embed = discord.Embed(
            description=f"**{user.name}** has been added to the blacklist.",
            color=0x9C84EF
        )
        embed.set_footer(
    ...
    @blacklist.command(
        base="blacklist",
        name="remove",
        description="Lets you remove a user from the bot's blacklist."
    )
    @app_commands.describe(user="The user you want to remove from the blacklist.")
    @checks.is_owner()
    async def blacklist_remove(self, context: Context, user: discord.User) -> None:
        """
        Lets you remove a user from the bot's blacklist.
        ...
        embed = discord.Embed(
            description=f"**{user.name}** has been removed from the blacklist.",
            color=0x9C84EF
        )
        embed.set_footer(
            text=f"There {'is' if total == 1 else 'are'} now {total} {'user' if total == 1 else 'users'} in the blacklist."
